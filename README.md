...
| `NEXT_PUBLIC_HUBSPOT_PORTAL_ID` | HubSpot portal ID for site tracking script |
| `HUBSPOT_ACCESS_TOKEN` | HubSpot Private App token (server-only) |
| `HUBSPOT_PORTAL_ID` | Portal ID for form submission API (optional) |
| `HUBSPOT_FORM_ID` | HubSpot form GUID for `/api/lead` submissions (optional) |
| `HUBSPOT_API_KEY` | Legacy alias for `HUBSPOT_ACCESS_TOKEN` |
